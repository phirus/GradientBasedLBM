...
Matrix3D::Matrix3D(double diag):matrix(boost::extents[19][19])
{
    for(int i = 0;i<19;i++)
    {
        for(int j=0;j<19;j++)
...
Matrix3D::Matrix3D(const RelaxationPar3D &relax, bool forcingterm):matrix(boost::extents[19][19])
{
    for(int i = 0;i<19;i++){
        for(int j=0;j<19;j++){
            matrix[i][j] = 0;
    ...
    else
    {
        ...
        matrix[7][7] = 1 - 0.5;
        matrix[8][8] = 1 - 0.5 * relax.s_5;

        matrix[9][9] = 1 - 0.5 * relax.omega;
        matrix[10][10] = 1 - 0.5 * relax.s_11;
        matrix[11][11] = 1 - 0.5 * relax.omega;
        matrix[12][12] = 1 - 0.5 * relax.s_11;
        matrix[13][13] = 1 - 0.5 * relax.omega;
        matrix[14][14] = 1 - 0.5 * relax.omega;
        matrix[15][15] = 1 - 0.5 * relax.omega;
        matrix[16][16] = 1 - 0.5 * relax.s_17;
        matrix[17][17] = 1 - 0.5 * relax.s_17;
        matrix[18][18] = 1 - 0.5 * relax.s_17;
    }
}
...
const array3D Matrix3D::diagMult(const array3D &other) const
{
    array3D a;
    for (int i= 0; i<19;i++)
    {
...
const DistributionSetType3D Matrix3D::diagMult(const DistributionSetType3D &other) const
{
    DistributionSetType3D a;
