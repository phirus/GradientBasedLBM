...
typedef boost::array<array2D,2> DistributionSetType;         /// < merge two distributions into a single variable
...
{
    const DistributionSetType diff = {{array2D_diff(one[0],two[0]), array2D_diff(one[1],two[1])}};
    return diff;
};

inline const DistributionSetType distro_add(const DistributionSetType &one, const DistributionSetType &two)
{
    const DistributionSetType foo = {{array2D_add(one[0],two[0]),array2D_add(one[1],two[1])}};
    return foo;
};

inline const DistributionSetType distro_add_array2D(const DistributionSetType &one, const array2D &two)
{
    const DistributionSetType foo = {{array2D_add(one[0],two), array2D_add(one[1],two)}};
    return foo;
};

inline const DistributionSetType distro_times(const DistributionSetType &one, double factor)
{
    const DistributionSetType foo = {{array2D_times(one[0],factor), array2D_times(one[1],factor)}};    
    return foo;
};
